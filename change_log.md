...
Most recent at top.
  * Next release
    * Tests: the suite now runs on JUnit Jupiter (JUnit 5.14) instead of the
      ...
      verify` works as before, fuzzer failures still replay with
      `-Djunit.seed=<seed>`, and the nested-anchor tag balancing case that was
      hidden behind a `failingtest` prefix is now reported as skipped.  Based
      on PR #374 by strangelookingnerd.
